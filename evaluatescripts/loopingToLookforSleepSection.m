...
sleepState = sleepState(startdate:enddate);
datenums = datenums(startdate:enddate);
sleepState(sleepState > 0) = 1;
...
% if the person falls asleep again within 2 hours of the lasst time they 
...
plot(fragmentSleep, 0.1*ones(size(fragmentSleep)), 'kx')
plot(fragmentWake, 0.1*ones(size(fragmentWake)), 'kx')
...
wakeTimes(removeWake) = [];
% 
plot(napSleep, 0.1*ones(size(napSleep)), 'rx')
plot(napWake, 0.1*ones(size(napWake)), 'rx')
% 
