...
%%%%
% Loop through all the sleep times
numberOfAwakenings = [];
for n = 1 : length(sleepTime)
    % find all the data between bed time and wake time
    i = find(testdates > sleepTime(n) & testdates < wakeTime(n));
    nightStates = testsleep(i);
    nightDates = testdates(i);
    % find all sleep states equal to zero
    i = find(nightStates == 0);
    nightwakeDates = nightDates(i);
    plot(nightwakeDates, zeros(size(nightwakeDates)), 'ko')
end
