...
[filename, pathname] = uigetfile('*.*', 'Choose CSV file');
...
% i = find(datenums > datenum([2017 9 4 0 0 0]) & datenums < datenum([2017 9 6 0 0 0]));
% dt = dt(i);
% datenums = datenums(i);
...
for n = 2 : length(sleepTimes)-1
    ...
    % nap.
    if spanBefore > 3/24 & spanAfter > 3/24
        % Add the sleep and awakening times to the nap list
...
for n = 1 : length(bedTime) - 1
   ...
   if TST < 14
    xlswrite(fullfilename, row, 1, ['A' num2str(rownum)]);
    rownum = rownum + 1;
   end
